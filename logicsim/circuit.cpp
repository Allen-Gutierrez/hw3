    ...
    while( getline(inFile, line))
    {
        ...
        if(line == "GATES")
        {
            ...
            for(int i = 0;i<n;i++)
            {
                ...
                //Add code here to support the NOT gate type (added)
                if(s_type == "NOT")
                {
                    std::string s_input; 
                    getline(ss, s_input, ','); // get input and use getLine 
                    std::string s_output;
                    getline(ss, s_output, ','); // get output and use getLine
                    m_gates.push_back(new NotGate(m_wires[stoi(s_input)], m_wires[stoi(s_output)])); // push at the end 
                }
            }
        }
        if(line == "INJECT")
