...
Node* llfilter(Node* head, Comp pred)
{
    ...
    Node* temp_filter = llfilter( head->next, pred);

    // Checks to see if the val of head matches the pred
    if ( pred (head -> val) ){
        delete head; 
        return temp_filter;

    } 
    else { // if doesn't match, return the value of temp_filter
        head -> next = temp_filter;
        return head;
